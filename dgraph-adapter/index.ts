  ...
  return {
    async createUser(user: AdapterUser): Promise<AdapterUser> {
      try {
        ...
        const result = await c.run<{ user: AdapterUser[] } | null>(
          /* GraphQL */ `
        ...
        const createdUser = result?.user?.[0];
        if (!createdUser) {
    ...
    async deleteUser(userId: string): Promise<AdapterUser | null> {
      try {
        // First, fetch the user to be deleted along with their accounts and sessions
        ...
        if (user.accounts.length > 0 || user.sessions.length > 0) {
          await c.run<{ deleteAccount: { numUids: number }; deleteSession: { numUids: number } }>(
            /* GraphQL */ `
